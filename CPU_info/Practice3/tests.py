from CPU_info.Practice1.cpu_info import CPUInfo
from generator import Generator


def test_cpu_info():
    t = CPUInfo("AMD", "7 4700G", "AM4", [8, 8], [2.6, 3.64], 22)
    assert t._Manufacturer == "AMD"
    assert t._Model == "7 4700G"
    assert t._Socket == "AM4"
    assert t._Cores == [8, 8]
    assert t._Frequency == [2.6, 3.64]
    assert t._TDP == 22
...
def test_gen_single_types():
    ...
    assert isinstance(g_one._Manufacturer, str)
    assert isinstance(g_one._Model, str)
    assert isinstance(g_one._Socket, str)
    assert isinstance(g_one._Cores, list)
    assert isinstance(g_one._Frequency, list)
    assert isinstance(g_one._TDP, int)
